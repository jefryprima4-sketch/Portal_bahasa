...
export default async function AdminSettingsPage() {
  ...
  const stats = [
    ...
    { label: 'Total Kursus', value: courseCount || 0, icon: 'menu_book', color: 'bg-purple-50 text-purple-600' },
    { label: 'Kuis Selesai', value: submissionCount || 0, icon: 'assignment', color: 'bg-orange-50 text-orange-600' },
  ];

  return (
    <div className="p-8 lg:p-12 space-y-10 max-w-7xl mx-auto font-body">
      <section className="space-y-1">
        <h2 className="text-4xl font-headline font-extrabold text-primary tracking-tight italic">Pengaturan</h2>
        <p className="text-on-surface-variant font-medium">Ringkasan sistem dan akses cepat administratif.</p>
      </section>

      {/* Stats Grid */}
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((s) => (
          <div key={s.label} className="bg-white p-6 rounded-[2rem] whisper-shadow border border-slate-50 flex flex-col justify-between h-36">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <a href="/admin/courses" className="bg-white p-8 rounded-[2rem] whisper-shadow border border-slate-50 group hover:translate-x-2 transition-all flex items-start gap-4">
          ...
          <div>
            <p className="text-sm font-bold text-slate-900 group-hover:text-primary transition-colors">Kelola Kursus</p>
